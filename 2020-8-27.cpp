...
string gstate;                        // special function
int gLine ;
int Opt_array;
...
void Opt_Statement( ) ;
void Opt_Expression( ) ;
void Opt_Basic_expression( ) ;
void Opt_Rest_of_Identifier_started_basic_exp( ) ;
void Opt_Rest_of_PPMM_Identifier_started_basic_exp( ) ;
void Opt_Sign( string peek ) ;
void Opt_Actual_parameter_list( ) ;
void Opt_Assignment_operator( string token ) ;
void Opt_Romce_and_romloe( ) ;
void Opt_Rest_of_maybe_logical_OR_exp( ) ;
void Opt_Maybe_logical_AND_exp( ) ;
void Opt_Rest_of_maybe_logical_AND_exp( ) ;
void Opt_Maybe_bit_OR_exp( ) ;
void Opt_Rest_of_maybe_bit_OR_exp( ) ;
void Opt_Maybe_bit_ex_OR_exp( ) ;
void Opt_Rest_of_maybe_bit_ex_OR_exp( ) ;
void Opt_Maybe_bit_AND_exp( ) ;
void Opt_Rest_of_maybe_bit_AND_exp( ) ;
void Opt_Maybe_equality_exp( ) ;
void Opt_Rest_of_maybe_equality_exp( ) ;
void Opt_Maybe_relational_exp( ) ;
void Opt_Rest_of_maybe_relational_exp( ) ;
void Opt_Maybe_shift_exp( ) ;
void Opt_Rest_of_maybe_shift_exp( ) ;
void Opt_Maybe_additive_exp( ) ;
void Opt_Rest_of_maybe_additive_exp( ) ;
void Opt_Maybe_mult_exp( ) ;
void Opt_Rest_of_maybe_mult_exp( ) ;
void Opt_Unary_exp( ) ;
void Opt_Signed_unary_exp( ) ;
void Opt_Unsigned_unary_exp( ) ;
...
int main()
{
  ...
  while ( true ) {
    try {
      ...
      Opt_array = 0;
      if ( gstate != "Done" && gstate != "ListAllVariables" && gstate != "ListAllFunctions"
          && gstate != "ListVariable" && gstate != "ListFunction")
        Opt_Statement();
    } // try
    catch ( Exception1 * e ) {
...
void Opt_Statement() {
  if ( gToken_list[Opt_array] == ";" ) return ;
  else if ( gToken_list[Opt_array] == "return" ) {
    Opt_array = Opt_array + 1;
    if ( gToken_list[Opt_array] != ";") Opt_Expression();

    return ;
  } // else if
  else if ( gToken_list[Opt_array] == "{" ) ;//Opt_Compound_statement();
  else if ( gToken_list[Opt_array] == "if" ) {                        // if
    Opt_array = Opt_array + 1;     // "if" -> "("
    Opt_array = Opt_array + 1;     // "("  -> Expression()
    Opt_Expression( );

    Opt_array = Opt_array + 1;     // ")"  -> Statement()
    Opt_Statement();

    if ( gToken_list[Opt_array] == "else" ) {
      Opt_array = Opt_array + 1;   // "else" -> Statement()
      Opt_Statement();
    } // if

    return ;
  } // else if
  else if ( gToken_list[Opt_array] == "while" ) {
    Opt_array = Opt_array + 1;     // "while" -> "("                       // while
    Opt_array = Opt_array + 1;     // "("  -> Expression()
    Opt_Expression( );
  ...
  else if ( gToken_list[Opt_array] == "do" ) {
    Opt_array = Opt_array + 1;     // "do"  -> Statement()
    Opt_Statement();

    Opt_array = Opt_array + 1;     // "whlie"  -> "("
    Opt_array = Opt_array + 1;     // "("  -> Expression()
    Opt_Expression( );
    ...
    if ( gToken_list[Opt_array] == "cin" ) {
      Opt_array = Opt_array + 1;     // "cin"  -> ">>"
      Opt_array = Opt_array + 1;     // ">>"  -> Expression()

      Opt_Expression();
    } // if
    else if ( gToken_list[Opt_array] == "cout" ) {                 // deal with "cout"
      Opt_array = Opt_array + 1;     // "cout"  -> "<<"
      Opt_array = Opt_array + 1;     // "<<"  -> Expression()
...
void Opt_Expression( ) {
  Run( false, "Opt_Expression" ) ;
  Opt_Basic_expression() ;
  Opt_array = Opt_array + 1;
  while ( gToken_list[ Opt_array] == "," ) {
    Opt_Basic_expression() ;
    Opt_array = Opt_array + 1;
  } // while
} // Opt_Expression
...
void Opt_Basic_expression( ) {
  Run( false, "Opt_Basic_expression" ) ;
  if ( gToken_list[Opt_arry] == "++" || gToken_list[Opt_arry] == "--" ) {
    Opt_array = Opt_array + 1;
    // GetValue
    // Value++
  } // if
