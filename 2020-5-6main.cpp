...
void Rest_of_declarators( ){
  ...
  */
  ...
  temp_variable = { ut_variable.type, ut_variable.name, "" };
  if ( peek == "[" ) {
    token = GetToken();                    // "["
    ucode = ucode + token ;
    ...
    ucode = ucode + " " + token ;
    temp_variable = { ut_variable.type, ut_variable.name, token };
    token = GetToken();                    // "]"
  ...
  while ( peek == "," ) {
    ...
    if ( peek == "[" ) {
      ...
      temp_variable = { ut_variable.type, name, token };
      token = GetToken();                   // "]"
      if ( token != "]" ) Error( token );
      else ucode = ucode + " " + token;
    } // if
    else
      temp_variable = { ut_variable.type, name, "" };
  ...
  if ( uisfunction ) {     // declaration 瞷function
  ...
  else {                   // 惠璶declaration ( not in function )
    for ( int i = 0 ; i < Nsurevariable.size() ; i++ ) {
...
void Formal_parameter_list() {   // function 把计
...
bool SearchID( string token ) {
  ...
  if ( token == "if" || token == "else" || token == "while" || token == "do" || token == "return" ) throw new Exception2( token );
