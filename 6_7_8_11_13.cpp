...
int main()
{
  ...
  while ( true ) {
    try {
      ...
      /*
      for ( int i = 0 ; i < gInput_list.size() ; i++ ) {
        for ( int j = 0 ;  j < gInput_list[i].size() ; j++ )
          cout << gInput_list[i][j] << " ";
        cout << endl;
      } // for
      */
    } // try
  ...
  if ( peek == "void" || peek == "int" || peek == "char"
    ...
    if ( gToken_list[2] == "(" ) {
      ...
      for ( int i = 3 ; gToken_list[i] != ")" ;  ) {
        ...
        if ( Is_ID( gToken_list[i + 1] ) ) {
          temp.name = gToken_list[i + 1];
          i = i + 1;
        } // if
        else {
          temp.name = gToken_list[i + 2];
          i = i + 2;
        } // else

        if ( Is_Constant( gToken_list[ i + 1 ] ) ) {
          temp.size = gToken_list[ i + 2 ];
          i = i + 4;
        } // if
        else {
          i = i + 1;
...
void Declaration() {
  ...
  int num = gToken_list.size() - 1;
  Rest_of_declarators();
  for ( int i = num ; i < gToken_list.size() ; i++ ) {
    if ( gToken_list[i] != ";" ) {
      if ( gToken_list[i + 1] == "[" ) temp.size = gToken_list[i + 2];
    } // if
...
void Statement() {
  ...
  if ( peek == ";" ) {
    ...
    gToken_list.push_back( GetToken() );        // "do"                     // do
...
void ListFunction( string token ) {
  ...
  /*
  for ( int i = 0 ; i < gInput_list.size() ; i++ ) {
    if ( gInput_list[i][1] == token ) {
      string space = "";
      cout << gInput_list[i][0] << " " << gInput_list[i][1] << gInput_list[i][2] << " " ;

      for ( int j = 3 ; j < gInput_list[i].size() ; j++ ) {
        cout << gInput_list[i][j] ;

        if ( gInput_list[i][j] != "{" && gInput_list[i][j] != ";"
             && gInput_list[i][j] != "}" ) {
          if ( j + 1 < gInput_list[i].size() && gInput_list[i][j + 1] != "["
               && gInput_list[i][j + 1] != "++" && gInput_list[i][j + 1] != "--" )
            cout << " ";
        } // if

        if ( gInput_list[i][j] == "{" ) space = space + "  ";
